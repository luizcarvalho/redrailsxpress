...
include( ABSPATH . 'administracao/admin-header.php' );
?>
<div class="wrap">
...
RewriteRule ^' . ( $subdomain_install ? '' : '([_0-9a-zA-Z-]+/)?' ) . 'files/(.+) wp-includes/ms-files.php?file=$' . ( $subdomain_install ? 1 : 2 ) . ' [L]' . "\n";
		...
		if ( ! $subdomain_install )
			$htaccess_file .= "\n# add a trailing slash to /administracao\n" . 'RewriteRule ^([_0-9a-zA-Z-]+/)?administracao$ $1administracao/ [R=301,L]' . "\n";
...
<?php include( ABSPATH . 'administracao/admin-footer.php' ); ?>
