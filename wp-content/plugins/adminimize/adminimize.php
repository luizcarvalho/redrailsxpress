...
function _mw_adminimize_remove_dashboard() {
	...
	if ( $disabled_menu_all != '' || $disabled_submenu_all != '' ) {
		...
		if ( $redirect ) {
			...
			$_mw_adminimize_db_redirect_admin_url = get_option( 'siteurl' ) . '/administracao/';
			...
			if ( preg_match( '#administracao/?(index.php)?$#', $_SERVER['REQUEST_URI'] ) ) {
...
function _mw_adminimize_set_user_info() {
	...
	switch ( $_mw_adminimize_user_info) {
		...
		case 3:
			...
			$_mw_adminimize_admin_head .= "\t" . 'jQuery(document).ready(function() { jQuery(\'#user_info\' ).remove();';
			if ( $_mw_adminimize_ui_redirect == '1' ) {
				$_mw_adminimize_admin_head .= 'jQuery(\'div#wpcontent\' ).after(\'<div id="small_user_info"><p><a href="' . get_option( 'siteurl' ) . ( '/administracao/profile.php' ) . '">' . $user_identity . '</a> | <a href="' . get_option( 'siteurl' ) . wp_nonce_url( ( '/wp-login.php?action=logout&amp;redirect_to=' ) . get_option( 'siteurl' ), 'log-out' ) . '" title="' . __( 'Log Out' ) . '">' . __( 'Log Out' ) . '</a></p></div>\' ) });' . "\n";
			} else {
				$_mw_adminimize_admin_head .= 'jQuery(\'div#wpcontent\' ).after(\'<div id="small_user_info"><p><a href="' . get_option( 'siteurl' ) . ( '/administracao/profile.php' ) . '">' . $user_identity . '</a> | <a href="' . get_option( 'siteurl' ) . wp_nonce_url( ( '/wp-login.php?action=logout' ), 'log-out' ) . '" title="' . __( 'Log Out' ) . '">' . __( 'Log Out' ) . '</a></p></div>\' ) });' . "\n";
			}
			$_mw_adminimize_admin_head .= '</script>' . "\n";
			break;
	}
...
/**
 * include options-page in administracao
 */
